    ...
    try
{
    std::filesystem::path homeDir;
        ...
        std::filesystem::path userPath = homeDir / "NeuVC";
    #else
        homeDir = std::filesystem::path(std::getenv("HOME"));
        std::filesystem::path userPath = homeDir / "Library" / "NeuVC";
    #endif

    auto getModelPath = [&userPath](const std::string& name) -> std::filesystem::path {
        return userPath / "Assets" / "traced" / name;
    };

    hubert_model = torch::jit::load(getModelPath("hubert_traced.pt").string());
    hubert_model.to(device);

    rmvpe_model = torch::jit::load(getModelPath("rmvpe_traced.pt").string());
    rmvpe_model.to(device);

    load_net_g((userPath / "Assets" / "traced" / "rvc_trained" / "ariana_grande.pt").string());
}
...
void RVC::setModelPath(const std::string &path)
{
    load_net_g(path);
}
void RVC::setModelPath(const std::string &path)
{
    if (!std::filesystem::exists(path)) {
        std::cerr << "Errore: Il file del modello non esiste!\n";
        return;
    }
