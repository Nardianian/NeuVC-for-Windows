...
void NeuVCAudioProcessor::_runModel()
{
    if (mProcessMode == "python")
    {
        ...
        _putenv_s("PATH", "C:\\Users\\capoc\\AppData\\Local\\Programs\\Python\\Python313");
...
#else
        pythonCmd = "/opt/miniconda3/bin/python ";
#endif

        std::string command = "cd \"" + workDir.toStdString() + "\" && ";
        command += pythonCmd;
        command += "infer_cli.py";
        command += " --input_path \"" + getSourceAudioManager()->getRecordedFile().getFullPathName().toStdString() + "\"";
        command += " --opt_path \"" + getSourceAudioManager()->getRecordedFile().getFullPathName().toStdString() + "\"";
        command += " --model_name \"" + mModelPath.toStdString() + "\"";
        command += " --index_rate 0";
        command += " --device cpu";
    ...
    else if (mProcessMode == "c++")
    {
        ...
        std::vector<float> normalized_audio = normalizAudio(audio);
        std::vector<float> resampled_audio = resampleAudio(normalized_audio, 48000, 16000);
        ...
        int out_rate = round(convertedAudio.size() / (resampled_audio.size() / 16000.0));
